...
def prepare_labels(
    ...
    targets: Optional[List[str]] = None,   # default: whatever the saved encoders hold
    ...
    for target in targets:
        if label_encoders and target in label_encoders:
            ...
            # Fit new encoder
            encoder = LabelEncoder()
            # Refitting here would permute class indices relative to training.
            # Use the saved encoder; see 01_calibrate_model.prepare_labels.
            labels[target] = encoder.transform(metadata[target].values)
            encoders[target] = encoder
            num_classes[target] = len(encoder.classes_)
...
def main():
    ...
    _encoders_path = args.model.parent / "label_encoders.json"
    if _encoders_path.exists():
        ...
        _num_classes, _regression_tasks = split_tasks(_task_info)
    else:
