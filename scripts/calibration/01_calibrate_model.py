...
python scripts/calibration/01_calibrate_model.py \\
    ...
    --features data/splits_v5/train_matrix.pa.mat \\
...
IGNORE_INDEX = -100  # task not applicable to this run
OOV_INDEX = -1       # run has a label the model was never trained on
...
def prepare_labels(
    ...
    """Encode labels using the SAVED training encoders, not fresh ones.

    Refitting a LabelEncoder on the evaluation metadata assigns class indices by
    the order of classes *in that split*, which need not match the order the model
    was trained with. When it does not, every metric is computed against permuted
    labels and looks plausibly wrong rather than obviously broken. Load the saved
    encoder instead, mark labels the model never saw as out-of-vocabulary, and mark
    runs the task does not apply to as masked.
    """
    if encoders_path is None:
        raise ValueError(
            "prepare_labels needs the saved label_encoders.json. Refitting encoders "
            "on the evaluation metadata silently permutes class indices."
        )
    with open(encoders_path) as fh:
        saved = json.load(fh)
    if targets is None:
        targets = [t for t, d in saved.items() if isinstance(d, dict) and "classes" in d]
        logger.info(f"Targets taken from the saved encoders: {targets}")

    labels, encoders, num_classes = {}, {}, {}
    for target in targets:
        if target not in saved or "classes" not in saved[target]:
            logger.warning("%s absent from the saved encoders; skipping", target)
            continue
        classes = saved[target]["classes"]
        encoder = LabelEncoder()
        encoder.classes_ = np.array(classes)
        known = {c: i for i, c in enumerate(classes)}
        vals = metadata[target].values if target in metadata else np.full(len(metadata), None)
        enc = np.array([
            IGNORE_INDEX if (v is None or (isinstance(v, float) and np.isnan(v)))
            else known.get(v, OOV_INDEX)
            for v in vals
        ])
        labels[target] = enc
        encoders[target] = encoder
        num_classes[target] = len(classes)
        logger.info("%s: %d classes, %d scored, %d masked, %d out-of-vocabulary",
                    target, len(classes), int((enc >= 0).sum()),
                    int((enc == IGNORE_INDEX).sum()), int((enc == OOV_INDEX).sum()))
...
def main():
    ...
    # NOTE (P0): architecture is reconstructed from the saved training artefacts,
    # not from labels refitted on the evaluation metadata. Regression heads must be
    # declared via regression_tasks= or their sigmoid is silently dropped and every
    # regression prediction collapses to 1.0.
    # Still v7-incompatible beyond this point (see PROJECT.md P3): prepare_labels()
    # refits LabelEncoders on the evaluation metadata instead of loading the saved
    # ones, and its default target list is the v5 set (includes sample_type, omits
    # sample_age/latitude/longitude).
    _encoders_path = Path(args.model).parent / "label_encoders.json"
    if _encoders_path.exists():
        with open(_encoders_path) as _fh:
            _encoders_saved = json.load(_fh)
        _task_info = task_info_from_encoders(_encoders_saved)
        _num_classes, _regression_tasks = split_tasks(_task_info, config.get("task_types"))
    else:
        _num_classes, _regression_tasks = num_classes, []
